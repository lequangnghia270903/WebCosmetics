...
class CheckoutController extends Controller
{
    ...
    public function email($order_id){
     ...
     $email1 =  DB::table('tbl_shipping')->where('tbl_shipping.shipping_id',$shipping_id)
     ->join('tbl_product','tbl_product.product_id','=','tbl_shipping.product_id')->get();
    $email=Session::get('email');
     foreach($email1 as $key =>$value){
                    $name =$value->product_name;
                    $a =$value->shipping_name;
                    $em =$value->shipping_email;
                    $sdt =$value->shipping_phone;
                    
                    $id =$value->shipping_id;
                    $anh=$value->product_image;
                

                    $dc =$value->shipping_address;
